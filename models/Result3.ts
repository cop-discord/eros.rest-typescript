/* generated using openapi-typescript-codegen -- do no edit */
/* istanbul ignore file */
/* tslint:disable */
/* eslint-disable */
import type { TwitterDL__models__Timeline__Core1 } from './TwitterDL__models__Timeline__Core1';
import type { TwitterDL__models__Timeline__EditControl } from './TwitterDL__models__Timeline__EditControl';
import type { TwitterDL__models__Timeline__Legacy2 } from './TwitterDL__models__Timeline__Legacy2';
import type { TwitterDL__models__Timeline__Views } from './TwitterDL__models__Timeline__Views';
export type Result3 = {
    __typename?: string;
    rest_id?: string;
    core?: TwitterDL__models__Timeline__Core1;
    unmention_data?: Record<string, any>;
    edit_control?: TwitterDL__models__Timeline__EditControl;
    is_translatable?: boolean;
    views?: TwitterDL__models__Timeline__Views;
    source?: string;
    legacy?: TwitterDL__models__Timeline__Legacy2;
};

